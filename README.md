...
| 読む人 | 読むファイル |
|---|---|
| **AIエージェント**（Claude Code / Codex など） | **`AGENTS.md` → `docs/プロジェクトの理念と背景.md` → `docs/STATE.md`** の順に必ず読む |
...
| ファイル | 内容 |
|---|---|
| `AGENTS.md` | AIエージェント向け指示書。ルール・禁止事項・利用者の作業スタイル・データ構造 |
| `docs/プロジェクトの理念と背景.md` | **サイトが存在する理由。技術より先に読む** |
| `docs/STATE.md` | 現状・次にやること・決定事項の記録（作業日誌） |
| `docs/記事執筆ルール.md` | 文体・**SUSURUルール（法的安全）**・スコアリング・カラー |
| `docs/サイト構成.md` | サイトの構造とデータの持ち方の詳細リファレンス |
| `docs/作業手順とチェック方法.md` | 動作確認の手順と、実際にハマった落とし穴 |
| `docs/過去の失敗と教訓.md` | 実際に起きた失敗の記録と、プロジェクト通史 |
| `docs/データ元システム.md` | スプレッドシート・GAS・WordPressの詳細（廃止予定の記録） |
| `docs/GASとGitHubの役割分担.md` | GAS・スプレッドシート・GitHubの役割分担と、投稿先の差し替え |
| `docs/GitHub-Pages移行手順.md` | WordPress廃止・GitHub Pages移行の作業手順 |
| `docs/運営とマネタイズ.md` | アフィリエイト・サウナ大学・未完了タスク一覧 |
...
- **GAS・スプレッドシート・スマホアプリは残します。** 記事の投稿先だけWordPress→GitHubに変更
- `reviews/index.html` と `サイトマップ/index.html` は **自動生成ファイル**。手で編集しないこと
- `sauna-daigaku.html` は **独自構造の特殊ファイル**。一括処理の対象から必ず除外すること
- 🔴 **このリポジトリを公開にする前に、必ず `docs/GitHub-Pages移行手順.md` の冒頭を読むこと**
  （GASのデプロイキーがGit履歴に残っています）
